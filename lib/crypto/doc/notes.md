...
- Fix `crypto:hash_equals/2` and FIPS when crypto is statically linked to the beam (with `--enable-static-nifs` and `--disable-dynamic-ssl-lib`).
...
- Application `crypto` is auto loaded by `crypto:on_load` in interactive mode. This will ensure that the correct value of configuration parameter `fips_mode` is used to initialize OpenSSL if module `crypto` is called/loaded before the application `crypto` has been loaded. In embedded mode, module `crypto` will fail to load if the application has not been loaded.

  Own Id: OTP-20035 Aux Id: [PR-10830]

- Untested OpenSSL engine support removed on Windows.
